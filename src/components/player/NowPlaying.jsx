...
const nowPlayingTransition = {
  duration: 0.32,
  ease: [0.22, 1, 0.36, 1],
};

const dragReleaseTransition = {
  type: "spring",
  stiffness: 500,
  damping: 40,
  mass: 0.8,
...
export function NowPlaying() {
  ...
  const swipeHandlers = useSwipeable({
    onSwipedLeft: () => {
      if (!seekingRef.current && !lyricsOpen) {
        next();
      }
    },
    onSwipedRight: () => {
  ...
  const handleNowPlayingDragEnd = (_event, info) => {
    const distanceThreshold = window.innerHeight * 0.2;
    const velocityThreshold = 1000;

    const draggedFarEnough = info.offset.y >= distanceThreshold;
    const swipedFastEnough = info.velocity.y >= velocityThreshold;

    if (draggedFarEnough || swipedFastEnough) {
      closeNowPlaying();
    }
  };
    ...
    <AnimatePresence>
      {nowPlayingOpen && current && (
        <motion.div
          ...
          dragMomentum={false}
          onDragEnd={lyricsOpen ? undefined : handleNowPlayingDragEnd}
          {...(lyricsOpen ? {} : swipeHandlers)}
        ...
        >
          <div className="now-playing__handle-zone">
            ...
            <button
              ...
              aria-label={
                lyricsUnavailable ? "Lyrics unavailable" : "Show lyrics"
              }
