 ...
 * fallback, so this only runs if the user has supplied their own personal
 * access token in Settings — otherwise it's a silent no-op rather than a
 * network call that was always going to be rejected.
 */
export async function findDiscogsArtwork({ artist, album }) {
  ...
  try {
    ...
    return { provider: "discogs", mbid: null, artworkUrl: match.cover_image };
  } catch (err) {
